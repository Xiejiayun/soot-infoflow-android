...
public class AndroidEntryPointConstants {
    ...
    /**
     * 判断一个方法是否为生命周期方法，这里首先判断最常见的四种组件
     * （存在一个问题，有可能某个类并不是从Activity继承而来）
     * @param className
     * @param methodName
     * @return
     */
    public static boolean isLifecycleMethod(String className, String methodName) {
        boolean isLifeCycleMethod = false;
        if (!className.equals(ACTIVITYCLASS) && !className.equals(SERVICECLASS) && !className.equals(BROADCASTRECEIVERCLASS)
                && !className.equals(CONTENTPROVIDERCLASS))
            return isLifeCycleMethod;
        if (className.equals(ACTIVITYCLASS)) {
            for (String method : activityMethods) {
                if (methodName.contains(method))
                    return true;
            }
        }
        if (className.equals(SERVICECLASS)) {
            for (String method : serviceMethods) {
                if (methodName.contains(method))
                    return true;
            }
        }
        if (className.equals(BROADCASTRECEIVERCLASS)) {
            for (String method : broadcastMethods) {
                if (methodName.contains(method))
                    return true;
            }
        }
        if (className.equals(CONTENTPROVIDERCLASS)) {
            for (String method : contentproviderMethods) {
                if (methodName.contains(method))
                    return true;
            }
        }
